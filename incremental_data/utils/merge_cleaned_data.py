#!/usr/bin/env python3
"""
Merge existing 1k cleaned samples with new 1.5k incremental cleaned samples.
Creates combined 2.5k cleaned files for each dataset and length.

Usage:
    python incremental_data/utils/merge_cleaned_data.py \
        --existing_dir sft_data_cleaned \
        --incremental_dir sft_data_cleaned_incremental \
        --output_dir sft_data_cleaned_merged
"""
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Merge existing 1k cleaned samples with incremental 1.5k cleaned samples"
    )
    p.add_argument(
        "--existing_dir",
        required=True,
        help="Directory containing existing 1k cleaned samples (e.g., sft_data_cleaned/)",
    )
    p.add_argument(
        "--incremental_dir",
        required=True,
        help="Directory containing incremental 1.5k cleaned samples (e.g., sft_data_cleaned_incremental/)",
    )
    p.add_argument(
        "--output_dir",
        required=True,
        help="Output directory for merged 2.5k cleaned samples (e.g., sft_data_cleaned_merged/)",
    )
    p.add_argument(
        "--datasets",
        nargs="+",
...
def main():
    ...
    print("=" * 60)
    print("Merging Existing 1k + Incremental 1.5k Cleaned Data → Combined 2.5k")
    print("=" * 60)
    ...
    for dataset in args.datasets:
        for length in args.lengths:
            ...
            existing_pattern = (
                f"{dataset}/train-num_sample_1000-max_seq_{length}-*-cleaned.jsonl"
            )
            incremental_pattern = (
                f"{dataset}/train-num_sample_1500-max_seq_{length}-*-cleaned.jsonl"
            )
