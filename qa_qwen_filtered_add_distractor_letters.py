...
def parse_identifier_length(format_name: str, default_length: int = 8) -> int:
    """Resolve the identifier length from the format string."""
    if not format_name:
        return default_length
    for token in reversed(format_name.split("_")):
        if token.isdigit():
            return int(token)
    return default_length


def generate_letter_chain(num_identifiers: int, identifier_length: int):
    """Generate a chain of random alphabetic identifiers."""
    return [
        "".join(random.choices(string.ascii_letters, k=identifier_length))
...
def main():
    assert args.save_name in dataset_dict, f"Please choose a dataset from {dataset_dict.keys()}"
    ...
    identifier_length = parse_identifier_length(
        chain_distractor_config["format"], default_length=8
    )
    # for item in write_jsons:
    for item in tqdm(write_jsons):
        if distractor_type == "chain":
            chain_list = [
                generate_letter_chain(
                    chain_distractor_config["num_uuids"], identifier_length
                )
                for _ in range(chain_distractor_config["num_chains"])
            ]
            chain_string_list = []
            insert_input = True
